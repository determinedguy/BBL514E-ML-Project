...
sys.stdout = DualLogger(f"svm_evaluation_{timestamp}.txt")
...
def main():
    print("\n" + "="*50)
    print("STARTING ISOLATED SVM EVALUATION")
    print("="*50)
    ...
    print("\nDownsampling strictly to 2% for SVM to prevent RAM OOM crash...")
    # Extract a 2% slice strictly for the SVM memory constraints
    ...
    # cache_size=2000 strictly limits the math matrix to 2GB of RAM
    svm = SVC(kernel='rbf', cache_size=2000, random_state=config.RANDOM_STATE)
    svm.fit(X_svm, y_svm)
